...
class BO_algo(object):
    def __init__(self):
        ...
        constraint_model_kernel = ConstantKernel(constant_value=3.5) * RBF(length_scale=2.0, length_scale_bounds='fixed')
        objective_model_kernel = ConstantKernel(constant_value=1.5) * RBF(length_scale=1.5, length_scale_bounds='fixed')
        self.constraint_model = GaussianProcessRegressor(kernel=constraint_model_kernel, alpha=0.005**2)  # TODO : GP model for the constraint function
        self.objective_model = GaussianProcessRegressor(kernel=objective_model_kernel, alpha=0.01**2)  # TODO : GP model for your acquisition function
        x_0 = np.array([[0, 0]])
        self.previous_points.append([float(x_0[:, 0]), float(x_0[:, 1]), float(self.objective_model.predict(x_0)), float(self.constraint_model.predict(x_0))])
    ...
    def acquisition_function(self, x: np.ndarray) -> np.ndarray:
        ...
        # TODO: enter your code here
        x = np.atleast_2d(x)
        objective_model_mu, objective_model_sigma = self.objective_model.predict(x, return_std=True)
        previous_points = np.array(self.previous_points)

        valid_data_index = np.where(previous_points[:, 3] <= LAMBDA)[0]
        if len(valid_data_index) == 0:
            ei = 1
        else:
            t = np.min(previous_points[valid_data_index, 2])
            z = (t - objective_model_mu) / objective_model_sigma
            ei = objective_model_sigma * (z * norm.cdf(z) + norm.pdf(z))
        
        constraint_model_mu, constraint_model_sigma = self.constraint_model.predict(x, return_std=True)
        pr = norm.cdf(LAMBDA, loc=constraint_model_mu, scale=constraint_model_sigma)
    ...
    def add_data_point(self, x: np.ndarray, z: float, c: float):
        ...
        # TODO: enter your code here
        previous_points = np.array(self.previous_points)
        self.objective_model.fit(previous_points[:, :2], previous_points[:, 2])
        self.constraint_model.fit(previous_points[:, :2], previous_points[:, 3])
    ...
    def get_solution(self) -> np.ndarray:
        ...
        previous_points = np.array(self.previous_points)
        valid_data_index = np.where(previous_points[:, 3] <= LAMBDA)[0]
        x_opt_index = np.argmin(previous_points[valid_data_index, 2])
        return previous_points[valid_data_index[x_opt_index], :2]
