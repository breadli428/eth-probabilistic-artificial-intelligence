...
class Agent:
    ...
    def pi_update(self, data):
        ...
        loss_pi = torch.mul(-phi, logp).mean()
        loss_pi.backward()
    ...
    def v_update(self, data):
        ...
        for _ in range(100):
            val = self.ac.v(obs)
            loss_v = torch.sum(torch.square(val - ret))
            loss_v.backward()
    ...
    def get_action(self, obs):
        ...
        return self.ac.step(torch.as_tensor(obs, dtype=torch.float32))[0]
