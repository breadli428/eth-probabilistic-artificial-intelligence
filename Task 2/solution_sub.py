...
class BayesianLayer(nn.Module):
    ...
    def __init__(self, in_features: int, out_features: int, bias: bool = True):
        ...
        self.weights_prior = MultivariateDiagonalGaussian(mu=torch.zeros(self.out_features, self.in_features), rho=torch.ones(self.out_features, self.in_features) * (np.exp(-8) * 0.75 ** 2 + np.exp(-1) * 0.25 ** 2))
        ...
        #  )
        self.weights_var_posterior = MultivariateDiagonalGaussian(mu=torch.nn.Parameter(torch.Tensor(out_features, in_features).normal_(0., .1)), rho=torch.nn.Parameter(torch.Tensor(out_features, in_features).uniform_(-3., -3.)))
        ...
        if self.use_bias:
            ...
            self.bias_prior = MultivariateDiagonalGaussian(mu=torch.zeros(self.out_features), rho=torch.ones(self.out_features) * (np.exp(-8) * 0.75 ** 2 + np.exp(-1) * 0.25 ** 2))
            self.bias_var_posterior = MultivariateDiagonalGaussian(mu=torch.nn.Parameter(torch.Tensor(out_features).normal_(0., .1)), rho=torch.nn.Parameter(torch.Tensor(out_features).uniform_(-3., -3.)))
    ...
    def forward(self, inputs: torch.Tensor):
        ...
        if self.use_bias:
            ...
            log_prior = log_prior + self.bias_prior.log_likelihood(bias)
            log_variational_posterior = log_variational_posterior + self.bias_var_posterior.log_likelihood(bias)
        else:
            bias = None
...
class UnivariateGaussian(ParameterDistribution):
    ...
    def sample(self) -> torch.Tensor:
        ...
        weight_samples = self.mu + eps * self.sigma
...
class MultivariateDiagonalGaussian(ParameterDistribution):
    ...
    def log_likelihood(self, values: torch.Tensor) -> torch.Tensor:
        ...
        return torch.log(1 / (sigma * np.sqrt(2 * np.pi)) * torch.exp(-0.5 * ((values - self.mu) / sigma) ** 2)).sum()

    def sample(self) -> torch.Tensor:
        # TODO: Implement this
        sigma = torch.log(1 + torch.exp(self.rho))
        eps = torch.Tensor(self.mu.size()).normal_()
        weight_samples = torch.rand(self.mu.size())
        weight_samples = self.mu + eps * sigma
        return weight_samples
