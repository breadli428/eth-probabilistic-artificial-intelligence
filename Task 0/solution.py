...
def log_posterior_probs(x):
    ...
    for hypothesis_idx in range(3):
        for each in x:
            log_likelihood[hypothesis_idx] += HYPOTHESIS_SPACE[hypothesis_idx].logpdf(each)
        log_likelihood[hypothesis_idx] += np.log(PRIOR_PROBS[hypothesis_idx])    
    log_p = log_likelihood - logsumexp(log_likelihood)
